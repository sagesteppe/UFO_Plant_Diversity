...
```{r Interpolate FQI values to office }

# covariates: elevation, rolling window population density, 
# distance from nearest road, stratum
...
```{r Kriging interpolation of FQI across field office - prepare covariates, eval = F}
library(terra)
library(spatstat)
library(tigris)
...
popDen <- rast(file.path(rp, rf[grep('population', rf)] ))
...
popDen_c <- project(popDen_c, crs(ext), filename = 
...
flat_lines_psp <- as.psp(CO_roads)

L <- shift(flat_lines_psp, origin = "bottomleft")
unitname(L) <- "m"
D90m <- distmap(L, eps = 90)
...
# check for correlation amongst the independent variables from 5000 random cells
corr_check_sample <- terra::sample(fqa_stack, size = 5000, method = 'regular')

vif_results <- vifstep(corr_check_sample ,th=10)

rm(corr_check_sample, vif_results)
# this is the theoretical full model:
# glm(mCoC_r ~ distance_road * pop_density * patchsize_r * elevation * coordX * coordY, data = spatial_vars)

full_model <- glm(mCoC_r ~ ., data = spatial_vars)
optimal_models <- dredge(full_model, abbrev.names = T)

# gather the actual terms from the top performing models, to see what worked.
model_terms <- get.models(optimal_models)
