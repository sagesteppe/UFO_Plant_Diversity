...
stacked_model <- model.avg(optimal_models, subset = delta < 2.5, fit = T)
...
rm(model_6, model_8, model_2, model_4, model_16, model_14, oop, pred_20, pred_16, 
   pred_2, pred_4, pred_6, pred_8, model_terms)
```
...
rm(elev_90, popDen, patch, roadDist, x_rast, y_rast, fqa_results_V)
```

```{r remove some modelling variables}
rm(fqa_rast_vals, optimal_models, pred_stack)
```
