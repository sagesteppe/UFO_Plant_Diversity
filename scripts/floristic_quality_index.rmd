...
stacked_model <- model.avg(optimal_models, subset = delta < 2.11, fit = T)
...
From the predicted FQI values we can infer that habitat quality follows a consistent trend across the field office (Figure 2). In general, the lowest elevation sites display the lowest values of FQI and Mean-CoC, which is a direct result of low species diversity and composition. This is especially apparent between Montrose and Grand Junction on Highway 50. These and several other areas of the Salt Desert shrublands along Highway 50, and near Crawford are areas of concern. However, not all Salt Desert is inherently in this condition as can be seen from the values for a moderately sized parcel of BLM managed land immediately Southeast of Montrose; indicating that human uses of theses areas may be causal factors for these low values. Areas with low FQI values represent highlight potential restoration needs and the FQI prediction model can be a tool to help prioritize such actions. The highest elevation portions of BLM managed land are in the best habitat condition, and very good conditions range into Pinon-Juniper, including most of the disturbed areas around historic mining in the West End. By these metrics considerable portions of the sagebrush habitat occupied by Gunnison sage-grouse near Crawford are also in good condition
...
```{r optimal_models, out.width="50%", fig.cap= 'Contributions of each term to each model, and each model sized by contributions to ensemble prediction'}
