...
occurrences <- BIEN::BIEN_occurrence_species(spp_v, political.boundaries = F,
                                             new.world = T)
write.csv( file.path( praw, 'BIEN_occurrences.csv'), row.names = F)
```

```{r save bien occurrence records after 1900, eval = F}
...
```{r Calculate total geographic expanse}

onion <- bien1 %>% filter(scrubbed_species_binomial == 'Allium textile') %>% 
...
ag <- terra::rast(
  file.path(file.path(praw, 'landcover', r[grep('Agriculture', r)])) )
water <- terra::rast(
  file.path(file.path(praw, 'landcover', r[grep('OpenWater', r)])) )
snow <- terra::rast(
  file.path(file.path(praw, 'landcover', r[grep('SnowIce', r)])) )
urban <- terra::rast(
  file.path(file.path(praw, 'landcover', r[grep('Urban', r)])) )

impervious <- sum(ag, water, snow, urban)
impervious <- crop(impervious, rast_ext)
impervious <- classify(r, c(0, 0.75, 1), include.lowest=TRUE, brackets=TRUE)
plot(impervious)

rm(rast_ext, r, ag, water, snow, urban)
```
