...
rm(spp_clean_1, CO_plants, attributes)
```
...
Of the species identified by the Gaston and and Singleton method `r ` overlap with the species of conservation concern from above. 


```{r remove some writing variables}
rm(esa, fws_esa, blm, blm_ss, state, global, global_sub, state_sub, 
   state_sub1, state_aim, global_aim)
```
