...
abundant_species <- gastons %>% 
  filter(n > 50) %>% 
  slice_sample(n = 15, weight_by = n) %>% 
  arrange(n) %>% 
  mutate(y = rep(seq(1:5)/100, nrow(.)/5))

ggplot(gastons, aes(x = n)) +
  geom_density(fill = 'grey80') +
  geom_vline(xintercept = 1, color = 'red') +
  theme_bw() +
  labs(y = 'Density', x = 'Number of Plots Observed at', 
       title = 'Distribution of Species Richness Across Plots', 
       subtitle = '15 very common species randomly chosen as references') +
  geom_text(data = abundant_species, aes(x = n, y = y, label = SYMBOL_USDA),
            check_overlap = T)
