...
# AW_Bottoms -  what is wrong with you?
...
ahf <- select(all_hits_forb, 
       PLOTID, AoInference, Meeting, COVER_TYPE, ECO.SITE ) %>% 
  st_drop_geometry()
...
results <- expanse(r, byValue = T, unit = 'ha', transform = T)
pts <- st_read(
  '../../aimDB/data/raw/AIM_Sample_Design/AIM_Design_Stratification.shp',
                quiet = T)  %>% 
  select(PLOTID) %>% 
...
waff_dat2 <- achieving %>% 
  ...
  select(PLOTID, Estimate, stratum) 
