...
all_hits_forb <- left_join(all_hits_forb, bmf, by = 'ECO.SITE', 'COVER_TYPE') %>% 
...
all_hits_grass <- left_join(all_hits_tree, bmf, by = 'ECO.SITE') %>% 
  mutate(Meeting = if_else(Cover_HB >= TARGET, T, F))
...
```{r prep real data for overall ESD conditions, eval = F}
