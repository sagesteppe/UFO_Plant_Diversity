...
summary <- read.csv(file.path(praw, 'TerraDatVersionSummary.csv')) %>% 
  select(PrimaryKey, AH_NonNoxPerenForbCover:FH_NoxTreeCover, Latitude_NAD83:Longitude_NAD83)
...
```{r Calculate Covers based on updated hits}

attributes <- read.csv(file.path(ppro, files[grep('Attribute.*Table', files)]) )
lpi <-  left_join(lpi, attributes, by = 'SYMBOL_AIM') %>% 
  ...
  group_by(PrimaryKey, RecKey, PointNbr, FUNCTIONAL) %>% 
...
indicators <- read.csv( file.path(praw, f[grep('Functional', f)]) ) %>% 
  filter(INDICATOR_TYPE == 'MULTIPLE') %>% 
  select(LIFEFORM, FUNCTIONAL) %>% 
  mutate(across(.cols = everything(), ~ str_trim(.))) # %>% 
  #unite('f', FUNCTIONAL:LIFEFORM, sep = "=") %>% 
  #pull() # tried many variations on quo_expr no go
...
all_hits_w <- all_hits_w %>% 
  mutate(
    NonNoxPerenForbGrassCover = NonNoxPerenForbCover + NonNoxPerenGrassCover, # 1
    NonNoxAnnForbGrassCover = NonNoxAnnForbCover + NonNoxAnnGrassCover, # 2
    NonNoxCover = NonNoxPerenForbCover + NonNoxPerenGrassCover + 
      NonNoxAnnForbCover + NonNoxAnnGrassCover + NonNoxShrubCover + 
      NonNoxSubShrubCover + NonNoxSubShrubCover + NonNoxSucculentCover + 
      NonNoxTreeCover, # 3
    NoxPerenForbGrassCover = NonNoxAnnForbCover + NonNoxAnnGrassCover, # 4
    NoxAnnForbGrassCover = NonNoxAnnForbCover + NonNoxAnnGrassCover, # 5
    NoxCover = NoxPerenForbCover + NoxPerenGrassCover + NoxAnnForbCover +
      NoxAnnGrassCover  + NoxShrubCover + NoxSubShrubCover + NoxSubShrubCover
    + NoxSucculentCover + NoxTreeCover, # 6
    NonNoxShrubCover = NonNoxShrubCover + NonNoxSubShrubCover # not for everyone
  ) %>% 
  select(NonNoxPerenForbCover, NonNoxPerenGrassCover, 
         NonNoxShrubCover,
         NonNoxTreeCover, NonNoxSucculentCover) %>% 
  rename_with(~str_remove(., 'Cover')) %>% 
...
```{r Compare our results to AIM db Summary}
