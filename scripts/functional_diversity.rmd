...
waff_dat <- data.frame(
  ...
  Meeting = c(43, 53, 73, 82),
  Estimate = c(5, 6, 4, 3),
  Failing = c(52, 42, 23, 14)
) %>% 
  pivot_longer(cols = Meeting:Failing, names_to = 'Estimate', values_to = 'Percent') %>% 
  mutate(n = (Percent * Prop_area) / 10, 
         Stratum = factor(Stratum, levels = c('Sagebrush', 'Mtn. Shrub',
                                               'Salt Desert', 'Pinon-Juniper')))  # NEED ACRES REDUCED TO FAKE SMALL NUMBERS ...
...
ggplot(waff_dat, aes(fill = Estimate, values = n)) +
  ...
  scale_fill_manual('Condition', values = c('#808929', '#E0321F', '#1FE032')) +
  scale_x_discrete()  +
