...
```{r Overall Vegetation Benchmarks Chart Broken Down to ESD, fig.height=6, fig.cap = 'The Proportion of Plots in Each ESD which are meeting Reference Benchmarks'}
...
bind_rows(ach_forb, ach_shrub, ach_tree, ach_grass) %>% 
  ...
  map(., . %>% arrange(-Estimate.P) %>% 
        ...
        ungroup() %>% 
        bind_rows(., 
                  data.frame(Subpopulation = NA, 
                             estimate = 'None', 
                             Estimate.P = 100 - max(.$Estimate.P))
        ) %>% 
...
waff_dat2 <- achieving %>% 
  group_by(PLOTID) %>% 
  mutate(estimate = sum(str_detect(ACHIEVING, 'Y'))) %>% 
  ...
  left_join(., lkp, by = c('stratum' = 'old')) %>% 
  select(PLOTID, Estimate, Stratum, Prop_area) 
