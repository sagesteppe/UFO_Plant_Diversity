...
home_brew <- all_hits_w  %>% 
  mutate(Forb = sum(NonNoxAnnForbCover, NoxAnnForbCover, 
                             NonNoxPerenForbCover, NoxPerenForbCover),
         Grass = sum(NonNoxAnnGrassCover, NoxAnnGrassCover, 
                             NoxPerenGrassCover, NonNoxPerenGrassCover),
         PerenGrass = sum(NonNoxPerenGrassCover, NoxPerenGrassCover), 
         Shrub = sum(NonNoxShrubCover, NoxShrubCover, NoxSubShrubCover, 
                          NonNoxShrubCover),
         PerenForb = sum(NonNoxPerenForbCover, NoxPerenForbCover)
         ) %>% 
  rename(Tree = NonNoxTreeCover) %>% 
  select(PrimaryKey, Forb:PerenForb, Tree) %>% 
...
mean(off$difference) # 2.9 with OPPO included, # 2.6 without OPPO
off[off$Variable=='Shrub','Cover_HB']

cor(off[off$Variable=='Shrub','Cover_HB'], 
    off[off$Variable=='Shrub','Cover_TD'],
    method = "pearson") # much lower but still 0.97 pearson correlation 
...
compare <- inner_join(home_brew, terradat_calc, by = c('PrimaryKey', 'Variable')) %>% 
  mutate(across(where(is.numeric), ~ round(.x, 2)))


ggplot(data = compare, aes(x = Cover_TD, y = Cover_HB)) +
  geom_point() +
  facet_wrap(~Variable) +
  theme_bw() + 
  theme(aspect.ratio = 1) +
...
off <- compare %>% 
  ...
  filter(Cover_HB != Cover_TD) %>% 
  mutate(difference = abs(Cover_HB - Cover_TD))
# WE CAN COMPARE THESE VARIABLES TO OUT RESULTS, THEY SHOULD ALL EQUAL OUR RESULTS
