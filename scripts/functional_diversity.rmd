...
total_acres * % classification

we will recover the total area of each stratum

```{r Overall Vegetation Becnhmarks By Major Stratum, fig.height = 3, fig.cap = 'The Proportion of Land in Each Stratum Within Benchmark Conditions'}

waff_dat <- data.frame(
  Stratum = c('SS', 'SD', 'MMS', 'PJ'),
  ...
  mutate(n = (Percent * Prop_area) / 10) # NEED ACRES REDUCED TO FAKE SMALL NUMBERS ...

waf_values = c('Meeting' = '#1FE032', 'Failing' = '#E0321F', 'Estimate' = '#808929')

ggplot(waff_dat, aes(fill = Estimate, values = n)) +
  geom_waffle(color = "#2f4858", size = .25, 
              flip = TRUE ) +
  facet_wrap(~ Stratum, nrow = 1, strip.position = "bottom") +
  
  scale_fill_manual(values = c('#808929', '#E0321F', '#1FE032')) +
  scale_x_discrete()  +
  ...
  theme(
   ...
   labs(title = "Area Within Stratum Meeting Benchmark", 
        y = 'Acres')
