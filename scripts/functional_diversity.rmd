...
esg_cover <- read.csv(file.path(praw, f[grep('EcologicalSiteGroup', f)])) %>% 
  ...
  right_join(., esg_lkp_tab)

# make long

benchmarks <- esg_cover %>% 
  pivot_longer(cols = Grass:Tree, values_to = 'COVER', names_to = 'COVER_TYPE') %>% 
  mutate(ESTIMATE = 'LOWER', COVER_RANGE = 'ORIGINAL', MEAN = as.numeric(COVER), SET = 'ESG') %>% 
  bind_rows(., benchmarks) %>% 
  as.data.frame()

# now create full benchmarks

rm(lkp_abb, esg_lkp_tab)
```
