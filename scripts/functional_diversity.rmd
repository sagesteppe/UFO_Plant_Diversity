...
p1 <- ggplot(yn, aes(y = ECO.SITE, fill = ACHIEVING)) + 
  ...
  scale_fill_manual('Achieving', values = c('Y' = '#283B1F', 'N' = '#C9341C')) +
  theme_bw() + 
...
yn$title <- 'SAMPLE SIZE'
p2 <- ggplot(yn, aes(y = ECO.SITE, x = 1, fill = SAMPLE)) +
  facet_grid(. ~ title) +
  geom_tile() +
  scale_x_continuous(breaks = 5, guide = guide_axis(n.dodge = 2), ) +
  scale_fill_continuous('n', low="#8ecae6", high="#023047") +
  labs(x = NULL, y = NULL) +
  theme_bw() + 
  theme(
    legend.position="bottom", 
...
cowplot::plot_grid(p1, p2, rel_widths = c(0.81, 0.19))
```
