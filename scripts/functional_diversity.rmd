...
```{r Create compound metrics - ensure we are calculating values same as NOC team, fig.height=2}
...
bmf <- benchmarks %>% 
  ...
  rename('TARGET' = COVER) %>% 
  group_by(ECO.SITE) %>% 
  slice_min(COVER_RANGE, n = 1)
all_hits_forb <- left_join(all_hits_forb, bmf, by = 'ECO.SITE')

# which points are assigned where??

all_hits_forb <- read.csv( file.path(praw, f[grep('Interence', f)])) %>% 
  select(PLOTID = Plot.ID, AoInference, xcoord, ycoord) %>% 
  right_join(., all_hits_forb) %>% 
  rowwise() %>% 
  mutate(STRATUM = str_remove(str_extract(PLOTID, '^.*-'), '-'),
         Meeting = if_else(Cover_HB >= TARGET, T, F)) %>% 
  left_join(., wghts, by = c('STRATUM','AoInference'))  %>% 
  st_as_sf(coords = c('xcoord', 'ycoord'), crs = 4269)

all_hits_forb <- drop_na(all_hits_forb)

noxious_presence_analysis <- spsurvey::cat_analysis(
...
benchmarks %>% 
  ...
  ggplot() +
  geom_point(aes(x = COVER, y = fct_reorder(ECO.SITE, MEAN), color  = SET),# color = 'grey20', 
              shape = 18, alpha = 0.8, size = 3) + #benchmarks here
  geom_jitter(data = all_hits_forb, height = 0.2,
              aes(x = Cover_HB, y = fct_reorder(ECO.SITE, Cover_HB)), alpha = 0.4) +
   
#  geom_pointrange(data = , aes(xmin = lower, xmax = upper, x= mean, y = ECO.SITE),
...
```{r prep data for overall ESD conditions}
...
bm_herb <- benchmarks %>% 
  filter(COVER_TYPE %in% c('Grass', 'Forb') & ESTIMATE == 'LOWER' 
         & SET == 'ESD') %>% 
  select(ECO.SITE, COVER_TYPE, BM =  COVER) 
bm_herb_extra <- benchmarks %>% 
  filter(COVER_TYPE %in% c('Grass', 'Forb') & ESTIMATE == 'LOWER' 
         & SET == 'ESG') %>% 
  filter(!ECO.SITE %in% bm_herb$ECO.SITE) %>% 
  select(ECO.SITE, COVER_TYPE, BM =  COVER)

bm_wd <- benchmarks %>% 
  filter(COVER_TYPE %in% c('Tree', 'Shrub'), SET == 'ESD', ESTIMATE == 'UPPER') %>% 
  select(ECO.SITE, COVER_TYPE, BM = COVER) 

bm <- benchmarks %>% 
  filter(COVER_TYPE %in% c('Tree', 'Shrub'), SET == 'ESG') %>% 
  select(ECO.SITE, COVER_TYPE, BM = COVER)  %>% 
  filter(!ECO.SITE %in% bm_wd$ECO.SITE) %>% 
  bind_rows(., bm_herb_extra, bm_wd, bm_herb) %>% 
  rename(Variable = COVER_TYPE)

w <- left_join(all_hits_nat, bm, by = c('ECO.SITE', 'Variable'))
...
```{r Overall Vegetation Benchmarks Chart Broken Down to ESD, fig.height=8, fig.cap = 'The Proportion of Plots in Each ESD which are meeting Reference Benchmarks'}
...
rm(title, combineLegend, l2, l1, plots, plotLeg, p1, p2, yn, new_labels, yn_levls)
```
...
waff_dat <- data.frame(
  Stratum = c('Sagebrush', 'Salt Desert', 'Mtn. Shrub', 'Pinon-Juniper'),
  Prop_area = c(12, 20, 13, 55),
  None = c(12, 15, 17, 20),
  One = c(8, 9, 12, 18),
  Two = c(30, 28, 27, 22),
  Three = c(30, 26, 33, 10),
  ...
  pivot_longer(cols = None:Four, names_to = 'Estimate', values_to = 'Percent') %>% 
  mutate(n = (Percent * Prop_area) / 10, 
         Stratum = factor(Stratum, levels = c('Sagebrush', 'Mtn. Shrub',
                                               'Salt Desert', 'Pinon-Juniper')),
         Estimate = factor(Estimate, levels = c('Four', 'Three', 'Two', 'One', 'None')))
waff_dat <- data.table::setorder(waff_dat, Estimate)


zerofourPal <- c('None' = '#d7191c', 'One +' = '#fdae61', 'Two +' = '#ffffbf', 
                'Three +' = '#a6d96a', 'Four' = '#1a9641')
ggplot(waff_dat, aes(fill = Estimate, values = n)) +
  geom_waffle(color = "white", size = .25, 
              flip = TRUE ) +
  facet_wrap(~ Stratum, nrow = 1, strip.position = "bottom") +
  
  scale_fill_manual('Condition:', values = c('None' = '#d7191c', 'One' = '#fdae61', 'Two' = '#ffffbf', 
                'Three' = '#a6d96a', 'Four' = '#1a9641'),
...
```{r Total Area Meeting Benchmarks}
