...
rm(indicators, all_hits, all_hits_sum, lpi_sub)
```
...
ggplot(data = compare_agg, aes(x = Cover_TD, y = Cover_HB)) +
  geom_point() +
  labs(title = 'Validation of Calculations against Standards',
       x = 'TerrAdat', y = 'In house') +
  facet_wrap(~ Variable) +
  theme_bw() + 
...
rm(agg, lpi_agg, all_hits_agg, all_hits_agg_sum, terradat_calc, off, compare_agg)
```

we need TREE, SHRUB, NATIVE FORB, NATIVE GRASS for ESD's
...
rm(attributes, native_esd_grps, lpi_agg, all_hits_agg, lpi, attributes)
```
