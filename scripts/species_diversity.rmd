...
dominant_plants <- dominant_plants %>% 
  ...
  mutate(
    ...
    FUNCTIONAL_FINE = case_when(
      ...
      SYMBOL == c('PLIN7', 'PECR5') ~ 'PERENNIAL-FORB',
...
dominant_plants <- dominant_plants %>% 
  ...
  FUNCTIONAL = if_else(SYMBOL == 'ERUM', 'SHRUB', FUNCTIONAL)) 
...
```{r Number of Species per Ecological Site}
...
lifeform_fill_pal <- setNames(
  c('#37C864', '#30A934', '#AE0F86', '#860FAE', '#9A0F9A', '#DD4513', '#D4D7D4'),
  c('C3-ANNUAL-GRASS', 'PG', 'ANNUAL-FORB', 'PERENNIAL-FORB', 'FORB', 'TREE', 'SHRUB')
)

benchmarks %>% 
  ggplot() + 
  geom_bar(aes(x = Lifeform_count, colour = FUNC_LIFE, fill = FUNCTIONAL_FINE)) +
  scale_fill_manual(values = lifeform_fill_pal) +
  scale_color_manual( values = lifeform_pal) +
