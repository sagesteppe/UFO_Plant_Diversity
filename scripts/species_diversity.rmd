...
source('functions.R')
rm(strata_pal, ord, lifeform_fill_pal, boxplot_drawer, convex_hull_resample, theme_boxplot)
```
...
diversity <- beta_est %>% 
  ...
  left_join(., alpha_diver) %>% 
  st_as_sf()

rm(beta, vegdistR, alpha_diver, gamma_diver, beta_est)
```
