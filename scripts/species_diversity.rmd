...
ggdraw(add_sub(plot, "Ecological Site",  vpadding=grid::unit(0,"lines"), 
               y=4.8, x=0.5, vjust=4.5))
...
*Here we determine what proportion of species identified in ESD production tables are present at each AIM plot*
...
*We also combine plots by ES to determine $\gamma-diversity$ by site*
...
alpha_richness <- spp_richness %>% 
  group_by(PLOTID) %>% 
  count(name = 'Alpha') 

gamma_richness <- spp_richness %>% 
  group_by(ECO.SITE) %>% 
  distinct(SYMBOL) %>% 
  count(name = 'Gamma') %>% 
  filter(ECO.SITE != 'UNKNOWN')

# beta diversity for many sites, Sorenson dissimilarity
...
*beta diversity in species composition within each Ecological Site which has replicate plots*

```{r what is the turnover in both core functional and species richness between plots of same Ecological Site}

```


```{r feature engineer benchmarks for unsampled sites, eval = F}
