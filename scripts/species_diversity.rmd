...
The number of identified plant species varied widely by plot, from `r min(diversity$Alpha)` - `r max(diversity$Alpha)` (Med = `r median(diversity$Alpha)`). The `r nrow(alphaD)` plots with the most species in them were in `r nrow(unique(alphaD[!is.na(alphaD$ECO.SITE), 'ECO.SITE']))` Ecological Sites, this subset of plots had a minimum of `r min(alphaD$Alpha)` species per plot. Of the top `r nrow(alphaD)` plot subset `r nrow(ad)` the Ecological Sites with the highest observed $\alpha-diversity$ were: `r toString(ad)`. `r toString(ad_lfc)`of all plots had less than three-quarters as many species of forbs, grasses, and shrubs observed as were present in the production tables. `r toString(pull(ad_lfl[grep('FORB', ad_lfl$FUNCTIONAL_FINE), 'Obs']))` of all plots had fewer than three-quarters Annual Forbs, and Perennial Forbs, than expected. `r toString(pull(ad_lfl[grep('GRASS', ad_lfl$FUNCTIONAL_FINE), 'Obs']))` of all plots had fewer Annual C3 Grasses, Perennial C3 Grasses, C4 Perennial Grasses than expected. `r toString(pull(ad_lfl[grep('SHRUB', ad_lfl$FUNCTIONAL_FINE), 'Obs']))` of all plots had fewer Non-Resprouting Shrubs, Resprouting, and unclassified shrubs than expected. As previously mentioned no benchmarks can be formed from the attribute tables, however the lack of grasses is concerning, and indicates that not only is there a lack of cover of the functional cover (SECTION X.X.), but that this may be related to a lack of functional diversity brought on by a lack of species diversity. Concerns regarding the species composition of shrubs may be related to the prevalence of a few, weedy shrubs, notably *Guiterrhiza sarothae* perhaps preventing colonization of other shrubs across the landscapes.
...
The Ecological Sites with the greatest number of species have `r toString(gbs_c)` species per functional group. The number of forbs at these ecological sites range from `r toString(gbs_f[grep('forb', gbs_f)])`. The number of grasses at these sites range from `r toString(gbs_f[grep('grass', gbs_f)])`. The number of shrubs vary from `r toString(gbs_f[grep('shrub', gbs_f)])`.
