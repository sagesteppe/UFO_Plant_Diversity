...
```{r number of plots meeting benchmarks for known ESD}
...
f <- lifeform_coarse %>% 
  ...
  geom_point() + 
  geom_jitter(aes(y = Observed, color = METHOD), width = 0.2, alpha = 0.8) + 
  theme_bw() +
...
l <-  draw_label("Socio-economic measures")

ggdraw(add_sub(plot, "Ecological Site",  vpadding=grid::unit(0,"lines"), y=4.8, x=0.5, vjust=4.5))
  
rm(lpi_lifeform_esd, spp_richness_lifeform_esd)
```


While the plots of species per functional group exhibit a regular dispersion, we are not sure exactly how these data were collected, and use the median in lieu of the mean; which was quite similar and has negligible differences. 

```{r feature engineer benchmarks for unsampled sites}

feat_eng_C <- benchmarks %>% 
  group_by(FUNCTIONAL) %>% 
  summarise(No_spp = quantile(Lifeform_count, c(0, 0.25, 0.5, 0.75, 1)),
            q = c(0, 0.25, 0.5, 0.75, 1))

feat_eng_F <- benchmarks %>% 
  group_by(FUNCTIONAL_FINE) %>% 
  summarise(No_spp = quantile(Functional_count, c(0, 0.25, 0.5, 0.75, 1)), 
            q = c(0, 0.25, 0.5, 0.75, 1))

```


```{r Plot number of species at plots without benchmarks}

feat_eng_C %>% 
  filter(FUNCTIONAL != 'TREE') %>% 
  ggplot(aes(x = q, y = No_spp, color = q)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ FUNCTIONAL) +
  theme_bw() +
