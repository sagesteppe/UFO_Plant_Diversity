---
title: "Plant Functional Diversity - Species"
knit: (function(inputFile, encoding) 
  { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), 'Section_12.pdf')) })
output:
...
dp1 <- dominant_plants %>% 
  ...
  left_join(., attributes, by = c('SYMBOL' = 'SYMBOL_USDA'), multiple = "all") %>% 
...
In order to be able to make comparisons across functional groups, for each species present in both the ESD's and our plot based data, we ensured that they drew from the same attribute table. This was especially important for situations where subshrubs are alternatively classified as forbs or shrubs depending on context. After performing this, we recovered both the 'coarse' functional groups used by the AIM team, which correspond largely to the botanical notion of 'lifeforms'. Subsequently, using the developed attribute tables finer functional groups, developed locally, which reflect the propensity of major groups of these lifeforms to respond to various disturbances, such as drought (e.g. C3 and C4 grasses), and fire (non-resprout and re-sprouting shrubs). In instances where more fine scale functional data were not available, such as was the case for sprouting potential of many shrub species, then species were left at this level rather than attempting to determine an appropriate group for them.
...
rm(spp_richness_functional_esd, spp_richness_lifeform_esd, lpi_lifeform)
```
...
rm(beta, gamma_diver, alpha_diver, beta_est)
```
...
```{r Beta diversity by functional groups}

bc <- spp_richness %>% 
  st_drop_geometry() %>% 
  filter(ECO.SITE != 'UNKNOWN', GROWTHHABITSUB != 'TREE') %>% 
  group_by(ECO.SITE) %>% 
  mutate(Replicates = n_distinct(PLOTID),
         PRESENCE = 1) %>% 
  filter(Replicates >= 2) %>% 
  ungroup() %>% 
  select(PLOTID, SYMBOL, ECO.SITE, PRESENCE, GROWTHHABITSUB) %>% 
  split(., 
        interaction(.$ECO.SITE, .$GROWTHHABITSUB) )  %>% 
  map(. %>%     # arrange data into a binary matrix format required for the
        select(-ECO.SITE, -GROWTHHABITSUB) %>%  # function
        pivot_wider(
          names_from = 'SYMBOL',
          values_from = PRESENCE, 
          values_fill = 0, 
...
beta_coarse_f <- lapply(bc, vegdistR) %>% 
  bind_rows(., .id = 'ECO.SITE') %>% 
  pivot_longer(Plot1:Plot2, values_to = 'PLOTID')%>% 
  group_by(PLOTID, ECO.SITE) %>% 
  mutate(MeanBeta = mean(Sorenson)) %>% 
  select(-name, -Sorenson) %>% 
  distinct(PLOTID, ECO.SITE,  .keep_all = T) 

rm(bc)
```

In order to determine whether enough plots have been sampled to reliably estimate the number of species which are present per Ecological Site ($\gamma-diversity$), rarefaction and extrapolation curves from the iNEXT package were used with confidence intervals of 0.8, and 50 bootstrap replicates (@Hsieh2022iNEXT, @Chao2014iNEXT).
...
rarefaction <- spp_richness %>% 
  ...
  filter(Replicates >= 2) %>% 
  ungroup() %>% 
  dplyr::select(PLOTID, SYMBOL, ECO.SITE, PRESENCE) %>% 
  split(.$ECO.SITE) %>% 
  purrr::map(. %>%     # arrange data into a binary matrix format required for the
        select(-ECO.SITE) %>%  # function
        pivot_wider(
          names_from = 'SYMBOL',
          values_from = PRESENCE, 
...
rarefaction <- i_report(rarefaction, conf = 0.8, k = 25)
```


```{r Clear methods environment }
rm(f, fp, funct_names, lifeform_fill_pal, ord, ppro, praw, vegdistR)
```
...
While the species within any one reference table in an Ecological Site Descriptions are noted to represent the dominant species across a gradient which stratifies the Ecological Site, such as elevation, many of our plots contained an adequate number of these species per functional group. In fact, when considering the richness data, which we suspect is a more apt data set for comparing to the ESD taxa, many of our plots had the number of species expected in the reference table, for both the Forb and Shrub functional groups (Figure 2). For Shrubs it appears that the Ecological Sites which are meant to have the greatest number of shrub species do not reach these goals; and that perhaps only for these sites is the species representing the gradients of the Ecological Site utilized. Below we will make several comparisons of the number of species per plot to 3/4 of the number of species in each functional group in the production table. It appears unlikely that the production tables represent variables collected in a consistent manner; however they offer clear, and useful, insights. 
...
ad_lfc <- lifeform_coarse %>% 
  filter(METHOD == 'Richness') %>% 
  mutate(PASS = if_else(Observed > (Expected * 3/4), 1, 0)) %>% 
  group_by(FUNCTIONAL_C) %>% 
  summarise(Prop_P = 1- (sum(PASS)/n())) %>% 
  mutate(Obs = paste0(round(Prop_P * 100, 1), '% ')) %>% 
  pull(Obs)

ad_lfl <- lifeform_fine %>% 
  filter(METHOD == 'Richness') %>% 
  mutate(PASS = if_else(Observed > (Expected * 3/4), 1, 0)) %>% 
  group_by(FUNCTIONAL_FINE) %>% 
  drop_na(Expected) %>% 
  summarise(Prop_P = 1 - (sum(PASS)/n())) %>% 
  mutate(Obs = paste0(round(Prop_P * 100, 1), '%'),
         FF = 
           factor(FUNCTIONAL_FINE, 
                  levels = 
                    c('ANNUAL-FORB', 'PERENNIAL-FORB',
                      'C3-ANNUAL-GRASS', 'C3-PERENNIAL-GRASS', 'C4-PERENNIAL-GRASS',
                      'NON-RESPROUT-SHRUB', 'RESPROUT-SHRUB', 'SHRUB'))
         ) %>% 
  arrange(FF)
...
In regards to $\alpha-diversity$ , the number of identified plant species varied widely by plot, from `r min(diversity$Alpha)` - `r max(diversity$Alpha)` (Med = `r median(diversity$Alpha)`). The `r nrow(alphaD)` plots with the most species in them were in `r nrow(unique(alphaD[!is.na(alphaD$ECO.SITE), 'ECO.SITE']))` Ecological Sites, this subset of plots had a minimum of `r min(alphaD$Alpha)` species per plot. Of the top `r nrow(alphaD)` plot subset `r nrow(ad)` the Ecological Sites with the highest observed $\alpha-diversity$ were: `r toString(ad)`. The percent of all plots which had less than three-quarters the number of species noted, as were in the production table `r toString(ad_lfc)` for forbs, grasses, and shrubs respectively (Figure 2). The percent of all plots which had fewer than three-quarters the number of Annual and Perennial Forb species than in the production table were `r toString(pull(ad_lfl[grep('FORB', ad_lfl$FUNCTIONAL_FINE), 'Obs']))` (Figure 1).  The percent of all plots which had fewer than three-quarters the number of Annual C3 Grasses, Perennial C3 Grasses, C4 Perennial Grasses species in the production table were `r toString(pull(ad_lfl[grep('GRASS', ad_lfl$FUNCTIONAL_FINE), 'Obs']))`. The percent of plots with fewer than three-quarters Non-Resprouting Shrubs, Resprouting, and unclassified shrubs species as in the production table were `r toString(pull(ad_lfl[grep('SHRUB', ad_lfl$FUNCTIONAL_FINE), 'Obs']))`. As previously mentioned no benchmarks can be formed from the attribute tables, however the lack of grasses is concerning, and indicates that not only is there a lack of cover of this functional group (Section 11), but that this may be related to a lack of functional diversity brought on by a lack of species diversity. 

Concerns regarding the species composition of shrubs and grasses may be related to the prevalence of *Guiterrhiza sarothae* (Section 13 Figure 1). Empirical evidence supports a theory that *Guiterrhiza sarothae* recruits when surplus resources become available, and that this resource availability is spurred by a lack of resource utilization by species which exploit similar niches, such as Grasses and Shrubs (@davis2000fluctuating, @ralphs2011broom). The decline in these other species are likely due to disturbances (@ralphs2011broom). However, once established at moderate densities *Guiterrhiza sarothae* has a competitive advantage and inhibits recruitment of other grasses and shrubs (@ralphs2011broom).

```{r beta diversity results}

betaD <- diversity %>% 
  ungroup() %>% 
  slice_min(MeanBeta, n = 25) %>% 
  st_drop_geometry() %>% 
  group_by(ECO.SITE) %>% 
  add_count()
...
beta_cf_sum <- beta_coarse_f %>% 
  separate(ECO.SITE, c('ECO.SITE', 'FUNCTIONAL')) %>% 
  group_by(ECO.SITE, FUNCTIONAL) %>% 
  summarize(median = median(MeanBeta),
            mini = min(MeanBeta),
            maxi = max(MeanBeta),
            ) %>% 
  ungroup()
...
bcf <- beta_cf_sum %>% 
  group_by(FUNCTIONAL) %>% 
  summarize(med = median(median)) %>% 
  mutate(
    bd_c = paste0(tolower(FUNCTIONAL), ' (', round(med, digits = 2), ')') ) %>% 
  pull(bd_c)

rm(ad, alphaD, ad_lfl, ad_lfc)
```

The mean $\beta-diversity$, that is the dissimilarity between plots varied from `r round(min(m$MeanBeta),2)` - `r round(max(m$MeanBeta),2)`, with lower values indicating greater differences between plots; and where the Ecological site with the most dissimilar plots, on average, contained just over half (`r round(min(m$MeanBeta),2)`) of the same species as each other. The range of Sorenson-Dice values for the `r nrow(betaD)` most dissimilar plots were from `r round(min(betaD$MeanBeta), 2)` - `r round(max(betaD$MeanBeta),2)`.  The `r nrow(betaD)` most dissimilar plots were in `r nrow(unique(betaD[!is.na(betaD$ECO.SITE), 'ECO.SITE']))` Ecological Sites, with the following  having the most plots in them: `r toString(bd)`. The median of beta-diversity of the major functional groups varied from `r toString(bcf)`,  indicating that turnover in forbs is much greater than for the other two groups. This is to be expected given that there are a greater number of forb species than grasses and shrubs. Given the low amounts of $\alpha-diversity$ observed for grasses, these results may indicate that the same few grass species are being observed across plots. The turnover in species composition for the minor functional groups was not analyzed as more replicates appear to be required. 
...
gamma_by_site <- spp_richness %>% 
  st_drop_geometry() %>% 
  filter(ECO.SITE != 'UNKNOWN', GROWTHHABITSUB != 'TREE', 
         ECO.SITE %in% NoSppGamma$Eco.Site) %>% 
  
  group_by(ECO.SITE, GROWTHHABITSUB) %>% 
  mutate( FC = n_distinct(SYMBOL)) %>% 
  group_by(ECO.SITE, FUNCTIONAL_FINE) %>% 
  mutate( FF = n_distinct(SYMBOL)) %>% 
  select(ECO.SITE, GROWTHHABITSUB, FUNCTIONAL_FINE, FC, FF ) %>% 
  distinct() 

gbs_c <- gamma_by_site %>% 
  ungroup() %>% 
  distinct(ECO.SITE, GROWTHHABITSUB, FC) %>% 
  group_by(GROWTHHABITSUB) %>% 
  mutate(
    fd_total = paste0(tolower(GROWTHHABITSUB), ' (', min(FC), '-', max(FC), 
                   ' med. = ', median(FC), ')' )
  ) %>% 
  distinct(fd_total) %>% 
  arrange(GROWTHHABITSUB) %>% 
  pull(fd_total)

gbs_f <- gamma_by_site %>% 
  ungroup(ECO.SITE) %>% 
    mutate(
      ff_total = paste0(tolower(FUNCTIONAL_FINE), ' (', min(FF), '-', max(FF), 
                   ' med. = ', median(FF), ')' )
  ) %>% 
  ungroup() %>% 
  arrange(FUNCTIONAL_FINE) %>% 
  filter(ff_total != 'c4-annual-grass (1-1 med. = 1)') %>% 
  distinct(ff_total) %>% 
  pull(ff_total)

rm(bcf, beta_cf_sum, bd, betaD, beta_coarse_f)
```

The $\gamma-diversity$ of Ecological Sites varied widely in the number. However, the accuracy of the current estimates of $\gamma-diversity$ are limited by the number of replicates per Ecological Site. The estimated Sample coverage per Ecological Site ranged from `r min(rarefaction$SampleCoverage)` to `r max(rarefaction$SampleCoverage)` with a median of `r median(rarefaction$SampleCoverage)`, 80% confidence interval; in general it appears only 3-4 plots are required to gain Sample coverage estimates about 50%. In other words, we do not yet have enough replicates per Ecological Site to make definitive statements regarding the species richness of each site, but clear trends have emerged. The Ecological Sites with the greatest number of species are `r toString(NoSppGamma$NoSppObs)`. The predicted estimates of the number of species which are to be expected, in the UFO, at these sites with further sampling are: `r toString(NoSppGamma$NoSppPred)` (80% confidence intervals). It appears that the high rates of $\gamma-diversity$  observed at two Ecological sites: R036XY114CO, R036XY326CO, are likely to relate to the large geographic expanses of them, rather than high rates of $\beta-diversity$.  While for the other sites, as expected $\beta-diversity$, is the primary factor fostering high rates of $\gamma-diversity$.  

The Ecological Sites above, which have the greatest net number of species have `r toString(gbs_c)` species per functional group. The number of forbs at these ecological sites range from `r toString(gbs_f[grep('forb', gbs_f)])`. The number of grasses at these sites range from `r toString(gbs_f[grep('grass', gbs_f)])`. The number of shrubs vary from `r toString(gbs_f[grep('shrub', gbs_f)])`, (Figure 1).

In summary, it appears that grass functional and taxonomic diversity are much lower than expected. It also appears that shrub diversity is wanting. Monitoring of forb diversity should be followed up several years post drought to determine if it rebounds. Returning to grasses, the number of species, in only a handful of plots, meets or exceeds the numbers of grass species noted present in reference conditions in all ESD's. Observations suggest that the grass species which appear to be missing from expanses of the field office include two species of *Sporobolus* (*S. airoides*, and *S. cryptandrus*), *Koeleria macrantha*, with noted declines in *Hesperostipa comata* and *Aristidia purpurea* in areas as well. 

```{r clear writing variables from environment}
rm(diversity, dominant_plants,
   lifeform_coarse, lifeform_fine, lpi, prop_lf_c, prop_lf_f, spp_richness,
   spp_richness_lifeform, spp_richness_clean, NoSppGamma, no_records_full, m, 
   rarefaction, i_report, gbs_f, gbs_c, gamma_by_site)
```
